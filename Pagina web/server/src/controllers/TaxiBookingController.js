...
export class TaxiBookingController {
  ...
  async validateAndAssignDriver(request, reply) {
    try {
      ...
      const booking = await this.bookingService.validateAndAssignDriver(
        bookingId,
        estadoReserva,
        rutConductor,
        patenteTaxi,
  ...
  async startTrip(request, reply) {
    try {
      ...
      const booking = await this.bookingService.startTrip(bookingId, driverId);
  ...
  async completeTrip(request, reply) {
    try {
      ...
      const result = await this.bookingService.completeTrip(
        bookingId,
        driverId,
        duracion,
        observacion
