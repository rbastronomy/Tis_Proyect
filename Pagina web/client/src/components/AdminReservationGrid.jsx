...
import { ChevronRight, MapPin, Calendar, DollarSign, User } from 'lucide-react'
...
export default function AdminReservationGrid() {
  ...
  const fetchAvailableDrivers = async () => {
    try {
      const response = await fetch('/api/users/drivers?status=ACTIVO', {
        credentials: 'include'
      })
      if (response.ok) {
        const data = await response.json()
  ...
  const handleConfirmAssignment = async () => {
    try {
      const response = await fetch(`/api/reservas/${selectedBooking.codigo_reserva}/assign`, {
        method: 'POST',
        headers: {
        ...
        credentials: 'include',
        body: JSON.stringify({
          rut_conductor: selectedBooking.rut_conductor
        })
      })
  ...
  const handleCardClick = (codigo_reserva) => {
    navigate({ to: `/admin/reservas/${codigo_reserva}` })
  }
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          ...
          <ModalBody>
            <Select
              label="Conductor"
              value={selectedBooking?.rut_conductor || ''}
          ...
          <ModalFooter>
            ...
            <Button 
              ...
              isDisabled={!selectedBooking?.rut_conductor}
            >
